"""
Test character update using FormData (exactly how frontend does it now)
"""
import requests
import time
import sys
...
def test_update_with_formdata():
    print("=" * 60)
    print("Testing Character Update with FormData (No Image)")
    print("=" * 60)
    ...
    # Update character using FormData (like the frontend now does)
    print("\n[4] Updating character using FormData (no image)...")

    # Build FormData exactly like frontend does
    form_data = {
        'class_name': 'Rogue',
        'race': 'Elf',
    ...
    print(f"    Response class: {updated.get('class', 'N/A')}")
    ...
    if success:
        ...
        print(f"    Expected class: Rogue, Got: {fetched.get('class')}")
