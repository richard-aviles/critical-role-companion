...
import { getPublicCampaign, getPublicCharacters, getPublicCampaignLayout, Character } from '@/lib/api';
...
function CharacterRosterPageContent() {
  ...
  // Fetch campaign, characters, and layout
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch campaign, characters, and layout in parallel
        const [campaignData, charactersData, layoutData] = await Promise.all([
          getPublicCampaign(slug),
          getPublicCharacters(slug),
          getPublicCampaignLayout(slug),
        ]);
      ...
      <div className="bg-white/95 dark:bg-gray-900/95 backdrop-blur-sm border-b border-gray-200 dark:border-gray-800 shadow-sm dark:shadow-lg">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400 mb-4">
            ...
            <span>/</span>
            <span className="text-purple-700 dark:text-purple-300 font-semibold">Characters</span>
          </div>

          {/* Title */}
          <h1 className="text-4xl font-bold bg-gradient-to-r from-purple-700 to-purple-900 dark:from-purple-400 dark:to-purple-200 bg-clip-text text-transparent">Character Roster</h1>
        ...
        {characters.length === 0 ? (
          ...
          <>
            ...
            {filteredCharacters.length === 0 ? (
              <div className="text-center py-12 bg-white/80 dark:bg-gray-900/80 backdrop-blur-sm rounded-xl shadow-md dark:shadow-lg border border-gray-200 dark:border-gray-800">
                <p className="text-gray-700 dark:text-gray-300 text-lg font-medium">
                  No characters match your search criteria.
                </p>
              </div>
            ) : (
              <div className={layout?.card_type === 'enhanced' ? 'grid grid-cols-1 md:grid-cols-2 gap-8' : 'grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6'}>
